...
const static int FD_NUM = 4;         // 两个文件描述符，分别为标准输入与输出
...
int main()
{
    ...
    while (1)
    {
        nev = kevent(kq, changes, 2, events, FD_NUM, NULL); // 已经就绪的文件描述符数量
        if (nev <= 0)
            quit("kevent()");
        ...
        for (i = 0; i < nev; i++)
        {
            ...
            // int ev_fd = (int)event.ident;

            // 输入流就绪 且 缓冲区还有空间能继续读
            if (ev_fd == stdin_fd && nread < BUFFER_SIZE)
            {
                std::cout << "输入事件触发" << std::endl;
                int new_nread;
